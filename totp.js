(function() {
  ...
  Totp = class Totp {
    ...
    getOtp(secret, now = new Date().getTime()) {
      ...
      hmac = CryptoJS.HmacSHA1(CryptoJS.enc.Hex.parse(time), CryptoJS.enc.Hex.parse(key)).toString(CryptoJS.enc.Hex);
  ...
  Hotp = class Hotp {
    ...
    getOtp(key, counter) {
      ...
      digest = CryptoJS.HmacSHA1(this.uintToString(new Uint8Array(this.intToBytes(counter))), key).toString(CryptoJS.enc.Hex);
      // Get byte array
